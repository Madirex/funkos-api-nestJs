...
export class UsersService {
  ...
  async deleteById(idUser: string) {
    ...
    if (!user) {
      throw new NotFoundException(`User con id ${idUser} no encontrado`)
    }
  ...
  async createOrder(createOrderDto: CreateOrderDto, userId: string) {
    ...
    if (createOrderDto.userId != userId) {
      throw new BadRequestException(
        'El userId del producto que se está adquiriendo debe de ser el mismo que el usuario autenticado',
      )
    }
