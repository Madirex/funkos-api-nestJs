...
describe('Util', () => {
  describe('getCurrentDateTimeString', () => {
    it('debería devolver una cadena con el formato correcto', () => {
      ...
      const pattern = /^\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2,3}$/
  ...
  describe('responseFunkoDtoAddPath', () => {
    it('debería agregar correctamente el path a la imagen', () => {
      const responseFunkoDto = { image: 'imagen.jpg' } as ResponseFunkoDto
      Util.responseFunkoDtoAddPath(responseFunkoDto)
      ...
      expect(responseFunkoDto.image).toBe(expectedPath)
    })

    it('no debería modificar el path si image es null o vacío', () => {
      const responseFunkoDto = { image: null } as ResponseFunkoDto
      Util.responseFunkoDtoAddPath(responseFunkoDto)
      expect(responseFunkoDto.image).toBeNull()
    })
  })
})
